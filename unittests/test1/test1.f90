program Test1
	...
	call cg_iric_read_integer(fid, 'intval', intval, ier)
	print *, 'intval: ', intval;
 
	call cg_iric_read_real(fid, 'doubleval', doubleval, ier)
	print *, 'doubleval: ', doubleval;
 
	call cg_iric_read_realsingle(fid, 'singleval', singleval, ier)
	print *, 'singleval: ', singleval;
 
	call cg_iric_read_string(fid, 'stringval', stringval, ier)
	print *, 'stringval: ', stringval;
 
	call cg_iric_read_functionalsize(fid, 'func1', func1size, ier);
	print *, 'func1size: ', func1size;
 
	allocate(func1time(func1size), func1value(func1size))
	call cg_iric_read_functional(fid, 'func1', func1time, func1value, ier)
	do i = 1, func1size
	...
	call cg_iric_read_functionalsize(fid, 'func2', func2size, ier);
	print *, 'func2size: ', func2size;
 
	allocate(func2time(func2size), func2value1(func2size), func2value2(func2size))
	call cg_iric_read_functionalwithname(fid, 'func2', 'time', func2time, ier)
	call cg_iric_read_functionalwithname(fid, 'func2', 'value1', func2value1, ier)
	call cg_iric_read_functionalwithname(fid, 'func2', 'value2', func2value2, ier)
	do i = 1, func2size
	...
	call cg_iric_read_functional_realsingle(fid, 'func1', func1timereal, func1valuereal, ier)
	do i = 1, func1size
	...
	! 格子を読み込む
	call cg_iRIC_Read_Grid2d_Coords(fid, grid_x, grid_y, ier)
	...
	! 属性を読み込む
	call cg_iric_read_grid_real_node(fid, 'node_real', node_real, ier)
	...
	call cg_iric_read_grid_integer_node(fid, 'node_int', node_int, ier)
	...
	! 属性を読み込む
	call cg_iric_read_grid_real_cell(fid, 'cell_real', cell_real, ier)
	...
	call cg_iric_read_grid_integer_cell(fid, 'cell_int', cell_int, ier)
	...
	! 境界条件の数を取得する
	call cg_iric_read_bc_count(fid, 'bctest', bctestcount)
	...
	do condid = 1, bctestcount
		call cg_iric_read_bc_indicessize(fid, 'bctest', condid, condindexlen(condid), ier)
		if (indexlenmax < condindexlen(condid)) then
			indexlenmax = condindexlen(condid)
		end if
		call cg_iric_read_bc_functionalsize(fid, 'bctest', condid, 'funcparam', func_size(condid), ier)
	...
	! インデックスと、境界条件 を読み込み
	do condid = 1, bctestcount
		call cg_iric_read_bc_indices(fid, 'bctest', condid, &
			condindices(condid:condid, 1:2, 1:condindexlen(condid)), ier)
		call cg_iric_read_bc_integer(fid, 'bctest', condid, &
			'intparam', intparam(condid), ier)
		call cg_iric_read_bc_real(fid, 'bctest', condid, &
			'realparam', realparam(condid), ier)
		call cg_iric_read_bc_realsingle(fid, 'bctest', condid, &
			'realparam', realsingleparam(condid), ier)
		call cg_iric_read_bc_string(fid, 'bctest', condid, &
			'stringparam', tmpstr, ier)
		stringparam(condid) = tmpstr
		call cg_iric_read_bc_functional(fid, 'bctest', condid, 'funcparam', &
			func_param(condid:condid, 1:func_size(condid)), &
			func_value(condid:condid, 1:func_size(condid)), ier)
		call cg_iric_read_bc_functional_realsingle(fid,  &
		...
		call cg_iric_read_bc_functionalwithname(fid,  &
	...
	do iter = 1, 5
		t = iter * 2.1
		call cg_iric_write_sol_time(fid, t, ier)
		result_biter_int = t * 3
		result_biter_real = t * 0.1
		call cg_iric_write_sol_baseiterative_integer(fid, 'result_biter_int', result_biter_int, ier)
		call cg_iric_write_sol_baseiterative_real(fid, 'result_biter_real', result_biter_real, ier)
		grid_x(1, 1) = grid_x(1, 1) + 0.1
		call cg_iRIC_Write_Sol_Grid2d_Coords(fid, grid_x, grid_y, ier)
		call cg_iRIC_Write_Sol_Node_Integer(fid, 'result_int', result_int, ier)
		call cg_iRIC_Write_Sol_Node_Real(fid, 'result_real', result_real, ier)
	end do
 
	call cg_iric_write_errorcode(fid, 2, ier)
