...
def writeSolution(filename, fid, iterMode):
    ...
    for i in range(5):
        ...
        fid = iric.cg_iRIC_Flush(filename, fid)
...
def readSolution(fid):
    ...
    for S in range(1, sol_count + 1):
        ...
        real_sol = iric.cg_iRIC_Read_Sol_Real_Mul(fid, S, "Depth")
        msg = "cg_iRIC_Read_Sol_Real_Mul() real_sol[0] == {0}".format(S - 1)
        util.verify_log(msg, real_sol[0] == S - 1)
        msg = "cg_iRIC_Read_Sol_Real_Mul() real_sol[{0}] == {1}".format(real_sol.size - 1, S - 1)
        util.verify_log(msg, real_sol[real_sol.size - 1] == S - 1)

        int_sol = iric.cg_iRIC_Read_Sol_Integer_Mul(fid, S, "IBC")
        msg = "cg_iRIC_Read_Sol_Integer_Mul() int_sol[0] == {0}".format(S - 1)
        util.verify_log(msg, int_sol[0] == S - 1)
        msg = "cg_iRIC_Read_Sol_Integer_Mul() int_sol[{0}] == {1}".format(int_sol.size - 1, S - 1)
