...
def cg_iRIC_Read_Grid_TriangleElementsSize2_WithGridId_Mul(fid, gid):
	ier, size = _iric.cg_iRIC_Read_Grid_TriangleElementsSize2_WithGridId_Mul(fid, gid)
	_checkErrorCode(ier)
	return size
...
def cg_iRIC_Read_Grid_TriangleElementsSize2_WithGridId(gid):
	ier, size = _iric.cg_iRIC_Read_Grid_TriangleElementsSize2_WithGridId(gid)
	_checkErrorCode(ier)
	return size
...
def cg_iRIC_Read_Grid_TriangleElementsSize2():
	ier, size = _iric.cg_iRIC_Read_Grid_TriangleElementsSize2()
	_checkErrorCode(ier)
	return size
...
def cg_iRIC_Read_Grid_TriangleElementsSize2_Mul(fid):
	ier, size = _iric.cg_iRIC_Read_Grid_TriangleElementsSize2_Mul(fid)
	_checkErrorCode(ier)
	return size
...
def cg_iRIC_Read_Grid_TriangleElements_WithGridId_Mul(fid, gid):
	size = cg_iRIC_Read_Grid_TriangleElementsSize2_WithGridId_Mul(fid, gid)
...
def cg_iRIC_Read_Grid_TriangleElements_WithGridId(gid):
	size = cg_iRIC_Read_Grid_TriangleElementsSize2_WithGridId(gid)
	id = IntArrayContainer(size)
...
def cg_iRIC_Read_Grid_TriangleElements():
	size = cg_iRIC_Read_Grid_TriangleElementsSize2()
	id = IntArrayContainer(size)
...
def cg_iRIC_Read_Sol_Grid3d_Coords(step):
	size = cg_iRIC_Read_Grid_NodeCount()
	x = RealArrayContainer(size)
	y = RealArrayContainer(size)
	z = RealArrayContainer(size)
	ier = _iric.cg_iRIC_Read_Sol_Grid3d_Coords(step, x, y, z)
	_checkErrorCode(ier)
	return x.get(), y.get(), z.get()

def cg_iRIC_Write_Sol_Grid2d_Coords(x_arr, y_arr):
	x = RealArrayContainer(x_arr.size)
	x.set(x_arr)
	y = RealArrayContainer(y_arr.size)
	y.set(y_arr)
...
def cg_iRIC_Read_Sol_Node_Real(step, name):
	size = cg_iRIC_Read_Grid_NodeCount()
	v = RealArrayContainer(size)
	ier = _iric.cg_iRIC_Read_Sol_Node_Real(step, name, v)
	_checkErrorCode(ier)
	return v.get()
...
def cg_iRIC_Read_Sol_Grid3d_Coords_WithGridId(gid, step):
	size = cg_iRIC_Read_Grid_NodeCount_WithGridId(gid)
	x = RealArrayContainer(size)
	y = RealArrayContainer(size)
	z = RealArrayContainer(size)
	ier = _iric.cg_iRIC_Read_Sol_Grid3d_Coords_WithGridId(gid, step, x, y, z)
	_checkErrorCode(ier)
	return x.get(), y.get(), z.get()

def cg_iRIC_Write_Sol_Grid2d_Coords_WithGridId(gid, x_arr, y_arr):
	x = RealArrayContainer(x_arr.size)
	x.set(x_arr)
	y = RealArrayContainer(y_arr.size)
	y.set(y_arr)
...
def cg_iRIC_Read_Sol_Node_Real_WithGridId(gid, step, name):
	size = cg_iRIC_Read_Grid_NodeCount_WithGridId(gid)
	v = RealArrayContainer(size)
	ier = _iric.cg_iRIC_Read_Sol_Node_Real_WithGridId(gid, step, name, v)
	_checkErrorCode(ier)
	return v.get()
...
def cg_iRIC_Read_Grid_TriangleElements_Mul(fid):
	size = cg_iRIC_Read_Grid_TriangleElementsSize2_Mul(fid)
	id = IntArrayContainer(size)
...
def cg_iRIC_Read_Sol_Grid3d_Coords_Mul(fid, step):
	size = cg_iRIC_Read_Grid_NodeCount_Mul(fid)
	x = RealArrayContainer(size)
	y = RealArrayContainer(size)
	z = RealArrayContainer(size)
	ier = _iric.cg_iRIC_Read_Sol_Grid3d_Coords_Mul(fid, step, x, y, z)
	_checkErrorCode(ier)
	return x.get(), y.get(), z.get()

def cg_iRIC_Write_Sol_Grid2d_Coords_Mul(fid, x_arr, y_arr):
	x = RealArrayContainer(x_arr.size)
	x.set(x_arr)
	y = RealArrayContainer(y_arr.size)
	y.set(y_arr)
...
def cg_iRIC_Read_Sol_Node_Real_Mul(fid, step, name):
	size = cg_iRIC_Read_Grid_NodeCount_Mul(fid)
	v = RealArrayContainer(size)
	ier = _iric.cg_iRIC_Read_Sol_Node_Real_Mul(fid, step, name, v)
	_checkErrorCode(ier)
	return v.get()
...
def cg_iRIC_Read_Sol_Grid3d_Coords_WithGridId_Mul(fid, gid, step):
	size = cg_iRIC_Read_Grid_NodeCount_WithGridId_Mul(fid, gid)
	x = RealArrayContainer(size)
	y = RealArrayContainer(size)
	z = RealArrayContainer(size)
	ier = _iric.cg_iRIC_Read_Sol_Grid3d_Coords_WithGridId_Mul(fid, gid, step, x, y, z)
	_checkErrorCode(ier)
	return x.get(), y.get(), z.get()

def cg_iRIC_Write_Sol_Grid2d_Coords_WithGridId_Mul(fid, gid, x_arr, y_arr):
	x = RealArrayContainer(x_arr.size)
	x.set(x_arr)
	y = RealArrayContainer(y_arr.size)
	y.set(y_arr)
...
def cg_iRIC_Read_Sol_Node_Real_WithGridId_Mul(fid, gid, step, name):
	size = cg_iRIC_Read_Grid_NodeCount_WithGridId_Mul(fid, gid)
	v = RealArrayContainer(size)
	ier = _iric.cg_iRIC_Read_Sol_Node_Real_WithGridId_Mul(fid, gid, step, name, v)
	_checkErrorCode(ier)
	return v.get()
